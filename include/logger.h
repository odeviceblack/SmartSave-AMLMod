...
public:
	...
	void SetFile(const char* szPath, const char* szFile);
	void SetFileFullPath(const char* szFullPath);
	void FlushFile();
...
private:
	...
	char m_szLogFile[128];
	char m_szFullPath[384];
	pthread_mutex_t m_fileMutex;

	void UpdateFullPath();
	void WriteToFile(eLogPrio prio, const char* szTag, const char* szMessage);
	void OpenLogFile();
	void CloseLogFile();
